...
const BookingPage = ({ params: { building, floor, room } }: any) => {
  ...
  const roomImage = building === 'block-a'
    ? blockARooms.find((check: RoomsType[number]) => check.id === room)?.image
    : blockBRooms.find((check: RoomsType[number]) => check.id === room)?.image;
    ...
    <>
      <div className='w-full h-screen flex items-center justify-center bg-black overflow-hidden'>
        <div className=' max-w-screen w-[80vh] max-h-screen h-[80vh] object-contain'>
          <img className=' object-cover h-full' src={roomImage} alt="" />
        </div>
      </div>
      <div className=" p-[2vh] rounded-md fixed top-[10dvh] right-[10dvh] bg-white">
        <div>
          <h4 className=' capitalize pb-[2vh] text-[2vh] mb-0'>{building} <VLine /> {floor} <VLine /> {room}</h4>
        </div>
        <Link href={`/${building}/${floor}/${room}/?booking=true`} className=' m-0 p-0 box-border' >
